...
def pi():
    ...
    for fold in range(CONFIG["n_splits"]):
        ...
        permutation_importance = PermutationImportance(model, random_state=2021).fit(x_val, y_val)
        feature_importances_mean = permutation_importance.feature_importances_
        feature_importances_std = permutation_importance.feature_importances_std_

        for idx, column in enumerate(train_columns):

            if column not in feature_score_map:
                feature_score_map[column] = [feature_importances_mean[idx] / CONFIG["n_splits"],
                                             feature_importances_std[idx] / CONFIG["n_splits"]]
            else:
                feature_score_map[column][0] += feature_importances_mean[idx] / CONFIG["n_splits"]
                feature_score_map[column][1] += feature_importances_std[idx] / CONFIG["n_splits"]

    # sort feature_score_map
    feature_score_map = {k: v for k, v in sorted(feature_score_map.items(), key=lambda item: item[1][0], reverse=True)}
